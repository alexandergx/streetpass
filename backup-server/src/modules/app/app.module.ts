...
export class AppModule {
  configure(consumer: MiddlewareConsumer) {
    // consumer.apply(createRateLimitGuard(Time.Minute * 5 * 1000, 1000)).forRoutes({ path: '*', method: RequestMethod.ALL, })
    // consumer.apply(createRateLimitGuard(Time.Minute * 1000, 300)).forRoutes({ path: '*', method: RequestMethod.ALL, })
